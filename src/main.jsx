...
function parseClientSheet(wb, sheetName, regimeFromPanel){
  ...
  for(let r=4; r<=15; r++){
    ...
    if(regime === 'MEI'){
      ...
      if(rec.status.startsWith('=')) rec.status = rec.das > 0 ? 'Pendente' : 'Pendente';
    } else {
...
function App(){
  ...
  async function loadFromSupabase(){
    ...
    if(e1){ setImportInfo('Erro em clientes: '+e1.message); setLoadingDb(false); return; }
    if(e2){ setImportInfo('Tabela lancamentos não encontrada. Execute o SQL atualizado.'); setLoadingDb(false); return; }
    ...
    setClients(grouped);
    setImportInfo(grouped.length ? `${grouped.length} clientes carregados do Supabase` : 'Banco conectado, mas ainda sem clientes importados');
    setLoadingDb(false);
  }

  async function saveToSupabase(parsed){
    if(!supabase){ setImportInfo('Importado localmente, mas Supabase não configurado.'); return; }
    setLoadingDb(true);
    ...
    const { error:e2 } = await supabase.from('lancamentos').upsert(movePayload, { onConflict:'cliente,mes' });
    if(e1 || e2) setImportInfo('Erro ao salvar: ' + (e1?.message || e2?.message));
    else setImportInfo(`${parsed.length} clientes e ${movePayload.length} lançamentos salvos no Supabase`);
    setLoadingDb(false);
  }
  ...
  async function handleExcelImport(e){
    ...
    if(parsed.length){ setClients(parsed); setImportInfo(`${parsed.length} clientes lidos da planilha. Salvando...`); await saveToSupabase(parsed); }
    else setImportInfo('Não encontrei abas de clientes na planilha.');
    e.target.value='';
  }
  ...
  return <div className={`app ${dark?'dark':''}`}>
    ...
    <main>
      ...
      {page==='Financeiro' && <section className='kpis'>{[['Recebido previsto',br(totals.fat*.08),'honorários estimados'],['Tributos',br(totals.das),'DAS/Guias'],['Pendências',totals.pend,'obrigações']].map(x=><Card key={x[0]}><span>{x[0]}</span><strong>{x[1]}</strong><small>{x[2]}</small></Card>)}</section>}
